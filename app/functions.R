...
## gene: the name of the gene of interest in the gene expression dataframe
# RETURN:
## A heatmap plot where the columns are arranged from lowest to highest Z-scaled expression of the @gene param, 
## and rows are arranged in alphabetical order of the genetic subtypes.
plot_heatmap_for_expr_gene <- function(gene) {
  expr_name <- str_glue("{gene}.expression")
    ...
    col <- scico(5, palette = "devon", direction = -1)
