...
ui <- fluidPage(
  ...
  h4("Which genes are included in the filtered gene expression dataset?"),
  p("Genes with low expression and low variance have been filtered out. Low expression genes are near the limit of detection for the RNA-seq gene expression quantification method, so the measurement of these genes will be low accuracy. Genes with low variance will not exhibit large changes between the mutated and unmutated patient groups, so they are less likely to provide reproducible results in follow-up experiments."),
  p("Filters applied to the gene expression dataset:"),
